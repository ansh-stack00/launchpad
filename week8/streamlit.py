...
st.set_page_config(page_title="Local GGUF LLM", layout="wide")
...
if mode == "Chat":
    ...
    if user_input:
        ...
        with st.chat_message("assistant"):
            ...
            for chunk in response.iter_content(chunk_size=1):
                if chunk:
                    assistant_text += chunk.decode("utf-8", errors="ignore")
    ...
    st.subheader("Single Prompt Mode")
    ...
    if st.button("Generate"):
        if not prompt.strip():
            ...
            response = requests.post(
                API_URL_GENERATE, json=payload, stream=True
            )
